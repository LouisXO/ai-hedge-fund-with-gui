...
**待预注册(评估点之前,不临时改规则)**
- 一美元以下股票能过内部人线的成交额门槛(9/24 的 BFRG:$0.95,跳空 −15%)。检验回测里 < $1 和 < $2 的事件是否拖累内部人线,预注册后再决定是否加价格下限。

**已讨论、未开始(需要用户说开始)**
- 竞价口径净值(S27b):每笔模拟成交按当天开盘竞价价重新记账,复盘/仪表盘/公开页并排显示两个口径。
- 大盘被动仓:把 $1 万期权预留改成 SPY 核心仓(S37 结论);评估点后再定三本书比例。lc_qlv 影子线接入 daily(记录不交易)。
