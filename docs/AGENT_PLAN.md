...
### S37(2026-09-24)— 大盘选股:九个规则**都不采用**;这段时间大盘里选股不加分
- 起因:v1 前 30 名市值中位数 17 亿,27/30 在 100 亿以下;用户要求大小盘都有。预注册(`agent/s37_largecap.py`,先提交再跑):时点市值 ≥ 100 亿(355–782 名,中位 579),30/60 等权,v1 同一引擎;候选 = 综合、rank-normal、质量+低波、质量+价值、纯动量、纯价值、综合+回购(净发行)、行业中性综合、综合 50/100;基线 lc_ew = 全部大盘等权。采用标准:alpha2 ≥ 3%/年且 t ≥ 2,超过 lc_ew ≥ 2%/年,β_size ≤ 0.3。
- 2017-01 → 2026-08 结果:没有一个过 t ≥ 2。lc_mom alpha2 +6.4%(t 0.87,β_size 0.37,换手 7.4,胜率 44%,2018 −9.9%);lc_qlv +2.1%(t 0.81)但 Sharpe 0.89、最大回撤 −30%、β_size −0.07、换手 1.6,是唯一真正"大盘且防守"的;lc_qv +2.8%(0.73)但 β_size 0.63;综合 +0.8%(0.23)、rankz +0.1%、回购 +0.0%、行业中性 +0.9%、综合 50 −0.9%;基线 lc_ew −0.9%(t −0.82)。CAGR 上除了纯动量(21%),所有大盘等权书都跑输 SPY 的 15.3%。
- 读法:这十年大盘的收益集中在少数超大盘(市值加权的 SPY 赢了所有等权的大盘篮子),四个经典因子在 ≥ 100 亿的股票里没有可测的选股边际,和文献(因子溢价在大盘最弱、2017 以后尤其)一致。v1 的边际来自小盘里的极端值,这在大盘里不存在。
- 处置:九个都不采用,Holm +9。**"大盘要有"的正确实现不是一本选股书,而是被动的大盘仓位**:模拟盘闲置资金本来就放 SPY;可以把预留的 $1 万期权额度改成 SPY 核心仓,评估点之后再讨论三本书的比例。lc_qlv 作为影子线记录(不交易),看它在实盘期的表现是否和回测一样防守。
- 下一步的大盘研究只有在拿到新数据时才值得做:分析师盈利预测修正(没有数据源)、财报后漂移在大盘已死(Martineau)、回购已测(无)。**不再用现有八个指标的组合去试大盘**。

### S27 + S27b(2026-09-24)— 执行成本:模拟器口径把长线 alpha 砍半;真实开盘竞价完全吃得下我们的量
- 数据:Alpaca `/v2/stocks/auctions`(免费档可查 15 分钟前的 SIP 历史)给每只股票每天的主交易所开盘/收盘集合竞价价格和成交量(`agent/sources/alpaca_auctions.py` → `auctions.db`)。**坑**:要用条件 "O"(开盘)/"6"(收盘)的那笔,"Q"/"M" 是各交易所各自的官方开收盘标记,可能是别的交易所的 1 股(MU 9/22 的 Arca "Q" 是 1 股,纳斯达克的真实开盘竞价 151,103 股);第一版用错了,得出"27% 的长线交易超过整个开盘竞价"的错误结论,已改。
- S27b(预注册,`agent/s27b_auction_capacity.py`):回测里两本书 2025-09 → 2026-08 的 704 笔交易,按模拟盘仓位(长线 $2,000、内部人 $1,500)算占开盘竞价的比例。长线中位 0.4%、p90 5%、> 25% 的 0 笔;内部人中位 0.5%、p90 3%、> 25% 的 2 笔。开盘竞价成交额中位:长线 $52 万、内部人 $29 万。**竞价价格 = 回测用的日线开盘价**(中位差 0.00%)。结论:**用真实账户下开盘单,这两本书按现在的规模完全能在开盘价成交**,两本都过预注册标准。
- S27(模拟盘实测 30 笔长线成交,平均 +0.61%/边,DAY 单逐笔撮合):把这个成本代进回测,长线 alpha2 从 +8.1%(t 1.46)降到 **+3.6%(t 0.65)**,CAGR 19.4% → 14.1%。内部人只有 2 笔成交,不够 20 笔,没跑。
- 读法:模拟盘不模拟竞价(S36),它的 +0.6%/边是模拟器的成本,不是真实账户会付的成本。**模拟盘的记录会系统性地比回测差约 4–5%/年**,评估点时必须按同一口径比较。
- 处置:下一步在账本里加一条"竞价口径净值"——每笔模拟成交按当天的开盘竞价价格重新记账(收盘后竞价数据可用),和模拟器口径并排显示。评估点按竞价口径判,模拟器口径作为保守下限。

### S38(2026-09-24)— 空头持仓做负面否决:**都不采用**;这几年高空头的反而跑赢
- 数据:FINRA 合并空头持仓 API(免费、无需 key,2020-04 起,每月两次;`agent/sources/finra_short.py` → `short.db`,310 万行),时点 = 结算日 + 9 个工作日。空头比例 = 空头股数 ÷ 最近申报的总股本。每周日随基本面任务更新。
- 预注册三个标记:空头比例 ≥ 20%、回补天数 ≥ 10、当天空头比例前 10%;做截面检验和两本书的入场否决(同 S33 的规则)。
- 截面(2020-06 → 2026-08,149 个公布日,h20):≥ 20% 的 −0.87%(t −0.66),回补 ≥ 10 天 −0.32%(−0.63),前 10% +0.03%(0.03)——**都不显著**,文献里的空头异象在这段时间没出现。
- 否决:长线书里被挡掉的高空头候选 h20 **+3.77%(t 3.05)**,前 10% 的 +2.53%(3.46);内部人书里回补 ≥ 10 天的 +0.78%(2.13)。方向和假设**相反**:我们两本书选中的高空头股票之后跑得更好(2021 年起的轧空环境),挡掉它们只会伤害收益。长线书 alpha 随否决升到 +17.6% 是样本内噪音(被挡的反而赚钱,alpha 升是因为替补者恰好更好,不采用)。
- 处置:六个都不采用,Holm +6。高空头**作为正面信号**没有预注册过,不从这次结果里挖;如果要测,另开一个预注册,并且只用 2026-10 以后的数据做样本外。

### S39(2026-09-24)— 增发 / 货架注册 / Form 144 做入场否决:**都不采用**;但"增发之后跑输"在全市场非常显著
- 数据:`agent/sources/sec_forms.py`(EDGAR 全文检索 → `filings.db`):424B5 与 S-3 从 2016-06 起,Form 144 从 2023-04 起(之前没有电子申报)。每天随收盘后任务更新。
- 全市场事件研究(下一个开盘起算,减 SPY):**424B5 增发定价后 20 天 −1.65%(t −4.31,只有 10% 的年份为正)**,5 天 −0.69%(t −3.16);货架注册后 20 天 −1.15%(t −3.24);Form 144 之后 5 天 −0.15%(t −1.94)、20 天 −0.14%(t −0.66)。增发是目前找到的最强的负面事件之一,和文献一致。
- 但落到我们两本书上:长线书候选里遇到增发的只有 942 次,之后 20 天 −0.88%(t −1.00);内部人书 475 次,5 天 −0.10%(t −0.20)。内部人书加上增发否决 alpha 从 +8.4%(t 1.93)升到 +9.0%(t 2.16),加货架否决升到 +9.6%(t 2.29),但被挡掉的股票本身没有显著跑输(t −1.62 最强),**不满足预注册的第二个条件**。Form 144 被挡掉的反而跑赢(长线 +1.87%,t 2.66;内部人 +1.16%,t 2.12),和 S38 一样方向相反。
- 处置:六个都不采用,Holm +6。书的 alpha 上升而被挡的股票不显著跑输,说明提升来自替补股票的运气,不是否决本身。增发的全市场负效应作为已知事实记下;如果以后要做,应该是"增发后做空"的独立事件线,我们不做空,所以不做。

### S36b(2026-09-24)— 内部人书:没成交的入场单重试一次
- 漏洞:`insider_targets` 跳过最近 8 天"下过单"的股票(本意是两天窗口里同一申报会出现两次,防止重复买),但没区分成交与否,于是 9/23、9/24 模拟器没成交的 12 笔(7 + 5)再也不会重试。
- 修正(`execute.blocking_orders`):只有"已成交"或"还挂着"的单才挡;成交量为 0 的终态单(过期/取消/拒绝)第二天重试**一次**,两次没成交就放弃。重试的单 `reason = entry_retry`,评估时单独统计。
- 依据:S31——晚一天开盘买平均 +0.31%(规则入场 +0.63%),真实账户扣成本后约打平,模拟器口径为负。所以重试是"不因为执行故障丢样本",不是"晚买也值得"。DAY 单(S36)之后没成交应该很少见。执行层修正,不进 Holm。
- 9/24 错过的 5 只不会被重试:触发它们的是 9/22–9/23 的申报,9/25 晚上的两天窗口已不包含。

### S40(2026-09-24)— 最低股价:长线书 $2 下限**过了预注册标准,但推迟到评估点的 v2 规则包**;内部人书不设下限
- 起因:BFRG($0.95,跳空 −15%)。预注册(`agent/s40_price_floor.py`):信号日收盘价低于 $1 / $2 / $5 不入场,两本书各测,2017-01 → 2026-08。
- 内部人线按价格分组(h5):< $1 −1.64%(t −0.59,70 次,前后半段 +2.78 / −6.05,极不稳定);**$1–2 +3.13%(t 2.98)**;$2–5 +0.84%(1.52);≥ $5 +0.24%(2.06)。低价股是这条线最赚钱的部分,不是拖累;BFRG 那样的 < $1 只有 70 次,方向不稳定。
- 内部人书:$1 下限 alpha +8.4% → +9.3%(t 2.18),但被挡的 70 次不显著(t −0.59),不采用;$2 下限 → +5.8%、$5 下限 → +2.3%,都在砍掉最赚钱的部分。**不设下限。**
- 长线书:$2 下限 alpha +8.1% → +8.9%(t 1.46 → 1.57),被挡的 286 次入场之后 20 天 **−4.60%(t −2.36)**,两个条件都满足,按 S40 的预注册应当采用。但 (1) 评估点规则(2026-09-23 预注册,优先级更高)规定评估点之前不改在跑的书;(2) t −2.36 在 Holm 家族(86+ 个变体)里过不了;(3) 前后半段 −7.34 / −1.86,效果在变弱;(4) 实际影响很小,10 年只挡 286 次。所以**记为"通过,列入评估点的 v2 规则包"**,和 S33 的 jump5、行业上限一起,到时一并预注册、一并决定。
- Holm +6。

### S41(2026-09-25)— 内部人线的首日路径:最近一年边际很薄,"当天盘中入场"不采用
- 数据:Alpaca 30 分钟线(`agent/sources/alpaca_intraday.py` → `intraday.db`,正常时段),最近 12 个月(2025-09 → 2026-08)全部 983 次入场都有 K 线。申报接受时间来自 SEC submissions API。
- Q1 首日路径(从 09:30 开盘起,减 SPY,按日聚类):10:00 +0.17%(t 1.31),10:30 +0.02%,13:00 +0.29%(t 1.43),15:00 +0.28%,收盘 +0.10%(t 0.51)。最近一年的首日盘中漂移很小且不显著;S31 的 +0.27%(t 4.7)是 2017–2026 全样本 11,304 次的结果,近一年明显弱于历史。之前 131 笔小样本探到的"开盘半小时 +0.54%"是抽样噪音,全样本不成立。
- Q2 当天盘中入场:1,882 份申报里只有 114 笔在 15:00 ET 前被 EDGAR 接受(**大多数 Form 4 在收盘后 16:00–18:00 ET 提交**,所以"当天买"对大部分事件根本不可用)。这 114 笔多出来的一段(申报当天盘中入场 → 次日开盘)平均 +0.19%,t 0.98,胜率 52%。没过 t ≥ 2 的门槛,**不采用**。
- 结论:开盘市价单仍是正确的入场方式;不需要盘中执行任务。执行层检验,不进 Holm。

### S42 + S42b(2026-09-25)— 用 5 分钟已实现波动率重做期权便宜度门:预注册标准通过,但**是陈旧成交价的伪影**,不采用
- 数据:5 分钟线(`intraday.db` bars5,561 只期权名,2023-12 起,2,286 万行)。每日已实现方差 = 5 分钟对数收益平方和(含隔夜),20/60 日均值年化。与日线 Yang-Zhang RV20 的相关只有 0.64。
- S42(预注册,`agent/s42_rv5_gate.py`,138,004 个标的日):S26 规则换成 5 分钟 RV 没有变化(pass − fail +0.48%,t 1.68);**比率门**(IV / rv5_20 在当天最低四分位)跨式 pass − fail h5 **+1.80%(t 5.80)**、h10 +1.60%(t 4.60),三年都为正,按预注册标准 KEEP。
- S42b(写在 S42 之后、跑之前的稳健性检验,`agent/s42b_rv5_gate_robustness.py`):期权"价格"是当天最后一笔成交;不活跃合约的陈旧低价会显示成"IV 便宜",下一笔成交回到公允价就成了假的买方收益。加上流动性过滤后效应消失:入场两腿成交量 ≥ 50 张且 ≥ 10 笔(F1)+0.35%(t 1.04);再要求出场成交量(F2)+0.43%(1.11);**按 VWAP 计价(F3)+0.05%(t 0.14)**;流动性最高的三分之一标的 +0.11%(t 0.26)。效应全部集中在低流动性合约的陈旧成交价上。
- 结论:**不采用**。S26 的结论不变,期权买方仍然没有验证过的入场。教训记入 AGENT.md:凡是用"最后成交价"算的期权收益,必须先过流动性和 VWAP 检验;S26 当年也是这样过的门。S42 的两个门计入信号级尝试数。
...
- v2 长线规则包(行业上限 20%、两簇显式化、S33 的 jump5 否决 +1%/年、S40 的 $2 最低股价):**评估点之后再预注册**,一起动,不分次动。
